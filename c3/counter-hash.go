...
func (app *CounterApp) Query(req types.RequestQuery) (rsp types.ResponseQuery){
  ver := req.Height
  if ver == 0 { ver = app.Version }
  rsp.Key = []byte("counter")
  value := app.History[ver]
  rsp.Value = []byte(fmt.Sprintf("",value))
  rsp.Height = ver
  rsp.Log = fmt.Sprintf("value@%v : %v",ver,value)
  return
}
